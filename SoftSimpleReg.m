...
epi{1} = ones(nx{1},ny{1})*c_eps_0; % commenting out this line and the next one for 2ci
epi{1}(125:150,55:95)= c_eps_0*11.3;

% New Inclusions
epi{1}(125:150,25:60) = c_eps_0*11.3; 
epi{1}(125:150,50:100) = c_eps_0*11.3;
epi{1}(125:150,47:74) = c_eps_0*11.3; 
...
% Boundary conditions of source #1 --> 2ciii
bc{1}.NumS = 3;                     % Number of sources = 3
bc{1}.s(1).xpos = nx{1}/(4) + 1;    % x position of source #1
bc{1}.s(1).type = 'ss';             % type of source #1
bc{1}.s(1).fct = @PlaneWaveBC;      % function of this source

% New source = source #2
bc{1}.s(2).xpos = nx{1}/(4) ; 
bc{1}.s(2).type = 'ss'; 
...
% Boundary Condition Type where 'a' = absorbing, 'e' would be electric
% field --> 2civ
bc{1}.xm.type = 'a';
bc{1}.xp.type = 'e';
bc{1}.ym.type = 'a';
